...
1. Write a View
create view standing as (select medal, year, height, event,
    coalesce(noc_region.region, case athlete_event.noc
...
select * from (select region, event,
...
select region, year, medal, count(*) as c, sum(count(*)) over (partition by region, medal order by year) as sum
from standing
...
select event, year, height, lag(height) over (partition by event order by year) as previous_height
from standing
where event like '%Pole Vault' and medal = 'Gold' and height is not null
order by event, year;